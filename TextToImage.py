...
def trainCNNBILSTM():
    ...
    text.insert(END,"CNN Bi-LSTM Model Accuracy : "+str(accuracy_value)) 
...
def graph():
    ...
    plt.grid(True)
    plt.xlabel('Traininng EPOCH')
    plt.ylabel('Accuracy/Loss')
    plt.plot(accuracy, 'ro-', color = 'green')
    plt.plot(loss, 'ro-', color = 'red')
    plt.legend(['Accuracy', 'Loss'], loc='upper left')
    plt.title('Training Accuracy & Loss Graph')
    plt.show()    


font = ('times', 14, 'bold')
title = Label(main, text='Generating Synthetic Images from Text using RNN & CNN')
...
trainButton = Button(main, text="Generate & Load RNN Model", command=trainCNNBILSTM)
trainButton.place(x=250,y=150)
trainButton.config(font=font1)

trainButton = Button(main, text="Training Accuracy & Loss Graph", command=graph)
trainButton.place(x=500,y=150)
trainButton.config(font=font1)
